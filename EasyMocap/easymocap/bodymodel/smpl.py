...
class SMPLModel(Model):
    ...
    def register_any_lbs(self, data):
        ...
        if self.use_pose_blending:
            ...
            if posedirs.ndim == 3:
                # Pose blend shape basis: 6890 x 3 x 207, reshaped to 207 x 20670
                num_pose_basis = posedirs.shape[-1]
                data['posedirs_origin'] = posedirs
                data['posedirs'] = np.reshape(posedirs, [-1, num_pose_basis]).T
            elif posedirs.ndim == 2:
                # Some SMPL npz packages already store posedirs as 207 x (6890*3).
                num_pose_basis = posedirs.shape[0]
                data['posedirs_origin'] = posedirs.T.reshape(self.v_template.shape[0], 3, num_pose_basis)
                data['posedirs'] = posedirs
            else:
                raise ValueError('Unsupported posedirs shape: {}'.format(posedirs.shape))
